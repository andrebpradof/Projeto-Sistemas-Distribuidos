...
module.exports = {
    ...
    async getHospital(call, callback){
        ...
        axios.get('https://maps.googleapis.com/maps/api/geocode/json?address='+cep+'&key=AIzaSyB-4lu2NPuYqsz6hTDNM8RTeDaa3B0raIE')
        .then(response => {
            if(response.data.status !== 'ZERO_RESULTS'){
                var {lat, lng} = response.data.results[0].geometry.location;
                
                axios.get('https://maps.googleapis.com/maps/api/place/nearbysearch/json?location='+lat.toString()+','+lng.toString()+'&radius=10000&type=hospital&key=AIzaSyB-4lu2NPuYqsz6hTDNM8RTeDaa3B0raIE')
                .then(response => {
